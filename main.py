            ...
            async with httpx.AsyncClient() as client:
                ...
                if resp.status_code == 200:
                    ...
                    async with db_pool.acquire() as conn:
                        ...
                        for p in posts:
                            # FIX: Har naye aane wale URL ko DB me save hone se pehle hi clean (unquote) kar lo
                            import urllib.parse
                            wp_slug = urllib.parse.unquote(p['slug'])
                            orig_url = urllib.parse.unquote(p['URL'])
...
async def server_side_rendered_blog(slug: str):
    ...
    # 6. Inject Recommendations DOM & Author Profile
    author_card_html = """
    <!-- Author Profile Card -->
    <div class="glass-morphism rounded-3xl p-6 md:p-8 mt-12 mb-2 flex flex-col md:flex-row items-center gap-6 md:gap-8">
        <div class="w-32 h-32 shrink-0 rounded-full p-1 bg-gradient-to-br from-green-400 to-green-600 shadow-lg">
            <img src="https://pranavblog.online/favicon.ico" alt="Pranav Sinha" onerror="this.onerror=null; this.src='https://ui-avatars.com/api/?name=Pranav+Sinha&background=10b981&color=fff&size=150';" class="w-full h-full object-cover rounded-full border-4 border-white pointer-events-none" style="-webkit-user-drag: none;">
        </div>
        <div class="text-center md:text-left flex-1">
            <h3 class="text-2xl font-bold text-green-950 flex items-center justify-center md:justify-start gap-2 mb-1">
                Pranav Sinha <svg class="w-5 h-5 text-blue-500" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path></svg>
            </h3>
            <p class="text-green-700 font-semibold text-[0.95rem] mb-1">Environmental Specialist & Climate Policy Expert</p>
            <p class="text-gray-500 text-sm font-semibold mb-4 flex items-center justify-center md:justify-start gap-1">
            ...
            <div class="flex flex-wrap justify-center md:justify-start gap-3 w-full">
                <a href="https://www.linkedin.com/in/pranav-sinha-a5002aa/" target="_blank" rel="noopener noreferrer" class="flex-1 md:flex-none inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-full bg-blue-50 text-blue-600 border border-blue-200 hover:bg-blue-600 hover:text-white transition-all font-bold text-sm shadow-sm">
                    <i class="fab fa-linkedin-in"></i> LinkedIn
                </a>
                <a href="mailto:pranavsinhain@gmail.com" class="flex-1 md:flex-none inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-full bg-red-50 text-red-600 border border-red-200 hover:bg-red-600 hover:text-white transition-all font-bold text-sm shadow-sm">
    ...
    html = html.replace('<!-- Recommendations Section -->', f'{author_card_html}\n            <!-- Recommendations Section -->')
