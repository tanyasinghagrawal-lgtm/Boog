...
async def flush_analytics_data():
    """Runs every 10 mins: Dumps RAM queue to Neon DB efficiently."""
    while True:
        ...
        try:
            async with neon_pool.acquire() as conn:
                ...
                for item in queue_snapshot:
                    ...
                    if ref not in INDEX_CACHE_REFS:
                        ...
                        INDEX_CACHE_REFS[ref] = row['id']

                # 2. Bulk Insert into main logs
                log_data = []
                for item in queue_snapshot:
                    ...
                    log_data.append((
                        ...
                        geo['country'],
                        geo['city'],
                        INDEX_CACHE_BLOGS[item['slug']],
                        INDEX_CACHE_REFS[item['referrer']],
                        item['visit_count'],
                        item['timestamp']
                    ))
                ...
                await conn.executemany("""
                    ...
                    (user_id, ip_address, country, city, blog_id, ref_id, visit_count, visited_at)
            ...
            # Put back in queue to retry next time
...
async def track_user_visit(data: TrackData, request: Request, background_tasks: BackgroundTasks):
    ...
    # Clean Referrer (if direct, save as Direct)
    clean_ref = data.referrer if data.referrer else "Direct"
    
    # Append to RAM Queue (Instant, Zero Latency)
    ...
    <script>
        setTimeout(function() {
            try {
                ...
                // Track total visits by this user
