...
async def serve_homepage():
    ...
    # 5-Minute RAM Cache for insane speed (Zero DB load on high traffic)
    if HOMEPAGE_CACHE["html"] and time.time() < HOMEPAGE_CACHE["expires"]:
        return HTMLResponse(content=HOMEPAGE_CACHE["html"])
        
    try:
        # Note: main4 wale index.html ko rename karke home.html kar lena server pe
        with open("home.html", "r", encoding="utf-8") as f:
            html_content = f.read()
    except FileNotFoundError:
        return HTMLResponse("<h1>home.html not found! Server pe main4 wale index.html ko rename karke home.html kardo.</h1>", status_code=404)
        
    # main5 ki table se direct data uthana (No extra scraping!)
    async with db_pool.acquire() as conn:
    ...
    if slider_track:
        ...
        for i, post in enumerate(slider_posts):
            # main5 ka image asset endpoint use ho raha hai yahan
    ...
    # Injecting Frontend JS for Slider & Infinite Scroll
    clean_js = """
    <script>
        ...
        function updateSlider() {
            ...
            if(indicatorsContainer) {
                ...
                for(let i=0; i<dots.length; i++) dots[i].className = (i===currentSlideIndex) ? 'h-1.5 rounded-full bg-white w-6 transition-all duration-300' : 'h-1.5 rounded-full bg-white/40 w-1.5 transition-all duration-300';
            }
        }
    ...
    final_html = str(soup)
    HOMEPAGE_CACHE["html"] = final_html
    HOMEPAGE_CACHE["expires"] = time.time() + 300 
...
async def server_side_rendered_blog(slug: str):
    """
    Core SSR Logic with Static Page Bypass.
    """
    import os
    from fastapi.responses import FileResponse
    
    # 1. STATIC PAGES BYPASS (Ye main4 ki property ab yaha merge ho gayi)
    ...
    # 2. Prepare 4 Recommendations (3 Related from AI Map, 1 Random)
    ...
    # Filter out current article and duplicates from all available
    valid_short_slugs = []
    for s in all_short_slugs:
        if s != current_short_slug:
            s_wp_slug = URL_MAP_CACHE[s].strip('/').split('/')[-1]
            if s_wp_slug not in DUPLICATE_SLUGS:
                valid_short_slugs.append(s)

    # --- NEW RECOMMENDATION ENGINE LOGIC ---
    final_recs_wp_slugs = []
    
    # A) Get Top 3 closest related articles from our Pre-computed AI MAP
    ...
    # B) Fill the remaining slot (4th spot) with a Random valid article
    random.shuffle(valid_short_slugs)
    for r_short in valid_short_slugs:
        if len(final_recs_wp_slugs) >= 4:
            break
    ...
    # Generate HTML for the 4 final recommendations
    rec_html_block = ""
    ...
    <style>
        ...
        /* 6. Ad Blocker UI CSS (Blur & Popup) */
    ...
    # 1. Scripts ko <head> tag ke turant baad inject karna (Fastest load)
    # Marked.js is used for beautiful Markdown rendering in AI chat, and ai-chat.js is our logic
    ...
    # 9. Inject Zero-Render-Blocking Analytics Tracker, Ad-Block Detector & Delayed Ads
    ...
    <script>
        ...
        function loadMainAd() {
            ...
            // Agar Ad-Blocker ne script load hone se block kar diya
        ...
        // Popup show karna aur 1-sec loop start karna
        function showAdBlockPopup() {
            let overlay = document.getElementById('adblockOverlay');
            overlay.classList.add('active');
            document.body.style.overflow = 'hidden'; // Scroll lock
            
            // Har 1 second me check karega background me
            if (!adCheckInterval) {
                adCheckInterval = setInterval(backgroundCheck, 1000);
            }
        }

        // Background me network check karega bina cache use kiye
        function backgroundCheck() {
            // Cache bypass ke liye har baar naya timestamp
            let testUrl = "https://richinfo.co/richpartners/in-page/js/richads-ob.js?pubid=1009153&siteid=394707&nocache=" + new Date().getTime();
            
            // Fetch use karke check karenge (ye script run nahi karega, bas network check karega)
            fetch(testUrl, { mode: 'no-cors', cache: 'no-store' })
            .then(() => {
                // Agar fetch pass hua (matlab Ad blocker band ho chuka hai)
                clearInterval(adCheckInterval); // Loop stop karo
                window.location.reload(true); // Page auto-refresh kardo ad load karne ke liye
            })
            .catch(() => {
                // Agar network error aaya (Ad blocker abhi bhi ON hai)
                // Kuch mat karo, loop chalne do aur popup dikhte rehne do
            });
        }

        // Page load hone ke 800ms baad pehla ad attempt
        setTimeout(loadMainAd, 800);

        // 2. Analytics Logic (Runs silently after 1.5s)
        setTimeout(function() {
            try {
