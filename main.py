...
async def fetch_all_mappings():
    ...
    try:
        # Use simple ssl context for asyncpg standard connection
...
async def db_proxy(websocket: WebSocket, path: str):
    ...
    writer = None
    try:
        # 3. Create SSL Context for Aiven with SNI support
        ssl_ctx = ssl.create_default_context()
        ssl_ctx.check_hostname = False # Trust Aiven certs (avoids self-signed errors)
        ssl_ctx.verify_mode = ssl.CERT_NONE
        
        logger.info(f"Connecting to Aiven DB at {db_host}:{db_port} (SNI Enabled)...")
        
        # 4. Open TCP Connection to Aiven - WITH SERVER_HOSTNAME (The Fix)
        # passing server_hostname ensures SNI is sent during handshake
        reader, writer = await asyncio.open_connection(
            host=db_host, 
            port=db_port, 
            ssl=ssl_ctx, 
            server_hostname=db_host, # <--- THIS LINE IS CRITICAL FOR AIVEN
        ...
        async def ws_to_tcp():
            ...
            try:
                while True:
                    ...
                    if not data:
                        break
                    # logger.info(f"WS -> DB: {len(data)} bytes") # Uncomment for deep debug
                    writer.write(data)
                    await writer.drain()
        ...
        async def tcp_to_ws():
            ...
            try:
                while True:
                    ...
                    if not data:
                        logger.info("Aiven closed connection (EOF).")
                        break
                    # logger.info(f"DB -> WS: {len(data)} bytes") # Uncomment for deep debug
                    await websocket.send_bytes(data)
            except Exception as e:
                logger.error(f"TCP->WS Error: {e}")

        # 6. Run pipes concurrently
        # return_when=asyncio.FIRST_COMPLETED ensures if one side dies, we close the other
