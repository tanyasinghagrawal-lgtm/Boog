...
async def server_side_rendered_blog(slug: str):
    ...
    for r_short_slug in random_recs:
        ...
        if rec_article:
            ...
            # Add polished alt text to recommendation image
            img_html = f'<div class="h-40 w-full overflow-hidden bg-gray-100"><img src="/img_asset/{r_hash}" alt="Read more about: {clean_r_title}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"></div>' if r_hash else '<div class="h-40 w-full bg-green-50 flex items-center justify-center"><i class="fas fa-leaf text-green-200 text-4xl"></i></div>'
    ...
    # --- FIX RENDER BLOCKING IN PYTHON MEMORY (No index.html changes needed) ---
    # 1. TailwindJS fix: Remove from top, add to bottom with 'defer'
    html = html.replace('<script src="https://cdn.tailwindcss.com"></script>', '')
    html = html.replace('</body>', '<script defer src="https://cdn.tailwindcss.com"></script>\n</body>')
    
    # 2. FontAwesome fix: Change to non-blocking preload
    old_fa = '<link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css" rel="stylesheet">'
    new_fa = '<link rel="preload" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css" as="style" onload="this.onload=null;this.rel=\'stylesheet\'"><noscript><link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css"></noscript>'
