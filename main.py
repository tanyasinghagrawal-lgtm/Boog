...
async def server_side_rendered_blog(slug: str):
    ...
    <div id="adblockOverlay" class="adblock-overlay">
        <div class="adblock-popup-content">
            ...
            <button onclick="checkAdBlocker(true)" class="adblock-btn">I Have Disabled It</button>
        </div>
    </div>

    <script>
        // 1. Ultimate Ad Blocker Detection (Active Script Loading)
        function checkAdBlocker(isRetry = false) {
            let scriptSrc = "https://richinfo.co/richpartners/in-page/js/richads-ob.js?pubid=1009153&siteid=394707";
            
            // Agar user retry kar raha hai, toh cache bypass karne ke liye timestamp add karenge
            if (isRetry) {
                scriptSrc += "&cb=" + new Date().getTime();
            }
            ...
            // Agar Ad-Blocker ne script ko block kiya (Network Request Failed)
            adScript.onerror = function() {
                overlay.classList.add('active');
                document.body.style.overflow = 'hidden'; // Background scroll lock
                adScript.remove(); // Clean up failed script
            };

            // Agar script successfully load ho gayi (Ad Blocker OFF hai, Ad chal gaya)
            adScript.onload = function() {
                overlay.classList.remove('active');
                document.body.style.overflow = ''; // Restore scroll
            };

            // Body me script append karo taaki actual load hona start ho
            document.body.appendChild(adScript);
        }

        // Page load hone ke 800ms baad script load karna start karo
        setTimeout(() => checkAdBlocker(false), 800);
