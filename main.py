...
async def get_query_embedding(query_text: str) -> Optional[List[float]]:
    """User ki search query ko naye text-embedding-004 model se convert karna."""
    url_template = "https://generativelanguage.googleapis.com/v1beta/models/text-embedding-004:embedContent?key={api_key}"
    payload = {
...
async def background_process_new_article_ai(wp_slug: str, html_content: str, title: str):
    """Background me naye article ke Single Vector aur AI Content generate karega."""
    logger.info(f"⏳ Starting background AI processing (Vectors & Content) for: {wp_slug}")
    
    soup = BeautifulSoup(html_content, 'html.parser')
    
    # NEW: Remove images from HTML before extracting text
