...
async def serve_compressed_image(slug: str):
    ...
    try:
        async with DB_POOL.acquire() as conn:
            # Join articles to get the image url associated with this slug
...
async def blog_viewer(slug: str):
    ...
    3. Builds HTML completely on server-side using BeautifulSoup.
    ...
    # 3. Server-Side HTML Rendering via BeautifulSoup
    try:
    ...
    for meta in soup.find_all("meta"):
        if meta.get("property") == "og:title":
            ...
            meta["content"] = "Read the latest article on Environment Energy and Agriculture."
    ...
    if local_image_path:
        ...
        if img_tag: img_tag["src"] = local_image_path

    body_tag = soup.find(id="blogBody")
    if body_tag:
        body_tag.clear()
        body_tag.append(BeautifulSoup(content, "html.parser"))
    ...
    if rec_grid:
        ...
        for rec in recs_data:
            img_html = f'<div class="h-40 w-full overflow-hidden bg-gray-100"><img src="{rec["image"]}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"></div>' if rec["image"] else '<div class="h-40 w-full bg-green-50 flex items-center justify-center"><i class="fas fa-leaf text-green-200 text-4xl"></i></div>'
            
            card_html = f"""
            <a href="/{rec['slug']}" class="glass-morphism glass-card rounded-xl overflow-hidden group block text-left">
                {img_html}
                <div class="p-5">
                    <h4 class="font-bold text-green-900 leading-tight mb-2 group-hover:text-green-700 transition-colors line-clamp-2">{rec['title']}</h4>
    ...
    # Make sure AD Script is injected natively to body since we removed frontend JS
    ...
    logger.info(f"Page generated and added to cache: /{slug}")
