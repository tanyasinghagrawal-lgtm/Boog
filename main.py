...
GEMINI_KEYS = [
    "AIzaSyDf8EAmsRMUq_BIacbDbBOC0d89GNcObqA",
    "AIzaSyDnMoERLM6CmVP7X29Zn9snmD3G9Wd840I",
    "AIzaSyDAv_tZec60X_f3LnRAlwW6YK9yWbFKnWU",
    "AIzaSyDesLPiyur26Q66RlmGPWYE8mVwO6UdbZE",
    "AIzaSyDDoTMaREyJHIe6whF8eS_UgExSFBipt2Q",
    "AIzaSyAYfne5JAdnd1olalaP1Amz6YcZUzyS5sk",
    "AIzaSyBk7mGCotL_SYed5MBPSSc7a41vll9Xjl4",
    "AIzaSyA0xqL5Rv0E6WXD7oX2SkdxknsneMmy-xQ"
]
...
async def generate_seo_description(html_content: str) -> Optional[str]:
    """Gemini AI se SEO description generate karna (with Key Rotation)."""
    global CURRENT_GEMINI_KEY_INDEX
    
    # HTML se sirf text nikalna (images & tags hata kar)
    soup = BeautifulSoup(html_content, 'html.parser')
    clean_text = soup.get_text(separator=" ", strip=True) # Safe limit for tokens
    
    prompt = "You are an expert SEO copywriter. Read the following blog text and generate a highly engaging, SEO-friendly meta description. It MUST be less than 150 characters. Do not use quotes or special characters that break HTML. Be direct, natural, and compelling.\n\nBlog Text:\n" + clean_text
        ...
        try:
            async with httpx.AsyncClient() as client:
                ...
                if resp.status_code == 200:
                    ...
                    return description[:155] # Safe cut
                else:
                    raise Exception(f"Status: {resp.status_code}, Resp: {resp.text}")
        except Exception as e:
            logger.warning(f"Gemini Key Index {CURRENT_GEMINI_KEY_INDEX} failed. Switching... Error: {e}")
            CURRENT_GEMINI_KEY_INDEX = (CURRENT_GEMINI_KEY_INDEX + 1) % len(GEMINI_KEYS)
            
    # Agar saari keys fail ho jayein
    await send_telegram_error("All Gemini API Keys failed while generating SEO description!")
    return None

async def background_update_seo_desc(wp_slug: str, html_content: str):
    """Background process jo missing description generate karke DB aur Cache me save karega."""
    seo_desc = await generate_seo_description(html_content)
    if seo_desc:
        ...
        try:
            ...
            logger.error(f"Failed to save background SEO desc: {e}")
...
async def init_db():
    ...
    try:
        ...
        async with db_pool.acquire() as conn:
            ...
            """)
            # Purani tables ke liye column dynamically add karna (taaki error na aaye)
            try:
...
async def scrape_and_save_article(wp_slug: str, client: httpx.AsyncClient) -> bool:
    ...
    try:
        ...
        seo_description = await generate_seo_description(processed_html)

        # Save to DB
        async with db_pool.acquire() as conn:
            await conn.execute("""
                INSERT INTO articles (wp_slug, title, content_html, featured_hash, seo_description)
                VALUES ($1, $2, $3, $4, $5)
                ON CONFLICT (wp_slug) DO UPDATE 
                SET title = EXCLUDED.title, content_html = EXCLUDED.content_html, featured_hash = EXCLUDED.featured_hash, seo_description = EXCLUDED.seo_description
            """, wp_slug, title, processed_html, featured_hash, seo_description)
...
async def startup_sync():
    ...
    async with db_pool.acquire() as conn:
        ...
        # Find which wp_slugs need scraping
        # Find which wp_slugs need scraping
        db_articles = await conn.fetch("SELECT wp_slug, title, content_html, featured_hash, seo_description FROM articles")
...
async def server_side_rendered_blog(slug: str):
    ...
    # --- SMART AI SEO DESCRIPTION LOGIC ---
    seo_desc = article.get("seo_description")
    
    if not seo_desc:
        # Fallback (Pehle jaisa simple extraction) taaki page render fast ho
        ...
        # Background task trigger karna taaki next time AI wala description aaye
        asyncio.create_task(background_update_seo_desc(wp_slug, article["content_html"]))
    else:
        # Double check cleaning for valid HTML injection
    ...
    # --- DYNAMIC AD INJECTION FOR RENDER ONLY (DO NOT SAVE TO DB) ---
    render_soup = BeautifulSoup(final_content_html, 'html.parser')
    
    # 300x250 (Usually fits both screens, gap increased)
    ad_after_p1 = BeautifulSoup("""<div class="w-full flex justify-center my-10 min-h-[250px] overflow-hidden"><iframe srcdoc="&lt;!DOCTYPE html&gt;&lt;html&gt;&lt;head&gt;&lt;style&gt;body{margin:0;padding:0;display:flex;justify-content:center;align-items:center;height:100%;}&lt;/style&gt;&lt;/head&gt;&lt;body&gt;&lt;script&gt;atOptions={key:'5cb5c1e8dfe4da83170e1b1982c9293b',format:'iframe',height:250,width:300,params:{}};&lt;/script&gt;&lt;script src='https://www.highperformanceformat.com/5cb5c1e8dfe4da83170e1b1982c9293b/invoke.js'&gt;&lt;/script&gt;&lt;/body&gt;&lt;/html&gt;" width="300" height="250" frameborder="0" scrolling="no" loading="lazy" style="max-width:100%;"></iframe></div>""", "html.parser")
    
    # 468x60 (Desktop) | 320x50 (Mobile)
    ad_mid = BeautifulSoup("""<div class="w-full my-10 overflow-hidden text-center">
        <div class="ad-desktop min-h-[60px]">
            <iframe srcdoc="&lt;!DOCTYPE html&gt;&lt;html&gt;&lt;head&gt;&lt;style&gt;body{margin:0;padding:0;display:flex;justify-content:center;align-items:center;height:100%;}&lt;/style&gt;&lt;/head&gt;&lt;body&gt;&lt;script&gt;atOptions={key:'969752f4dee8e05427cf25dde238cc40',format:'iframe',height:60,width:468,params:{}};&lt;/script&gt;&lt;script src='https://www.highperformanceformat.com/969752f4dee8e05427cf25dde238cc40/invoke.js'&gt;&lt;/script&gt;&lt;/body&gt;&lt;/html&gt;" width="468" height="60" frameborder="0" scrolling="no" loading="lazy" style="max-width:100%;"></iframe>
        </div>
        <div class="ad-mobile min-h-[50px]">
            <iframe srcdoc="&lt;!DOCTYPE html&gt;&lt;html&gt;&lt;head&gt;&lt;style&gt;body{margin:0;padding:0;display:flex;justify-content:center;align-items:center;height:100%;}&lt;/style&gt;&lt;/head&gt;&lt;body&gt;&lt;script&gt;atOptions={key:'556894ab6258448a0e6a40e709cacdc5',format:'iframe',height:50,width:320,params:{}};&lt;/script&gt;&lt;script src='https://www.highperformanceformat.com/556894ab6258448a0e6a40e709cacdc5/invoke.js'&gt;&lt;/script&gt;&lt;/body&gt;&lt;/html&gt;" width="320" height="50" frameborder="0" scrolling="no" loading="lazy" style="max-width:100%;"></iframe>
        </div>
    </div>""", "html.parser")
    
    # 728x90 (Desktop) | 320x50 (Mobile)
    ad_last_img = BeautifulSoup("""<div class="w-full my-10 overflow-hidden text-center">
        <div class="ad-desktop min-h-[90px]">
            <iframe srcdoc="&lt;!DOCTYPE html&gt;&lt;html&gt;&lt;head&gt;&lt;style&gt;body{margin:0;padding:0;display:flex;justify-content:center;align-items:center;height:100%;}&lt;/style&gt;&lt;/head&gt;&lt;body&gt;&lt;script&gt;atOptions={key:'cdc306a38190dff969b12ccb0f570436',format:'iframe',height:90,width:728,params:{}};&lt;/script&gt;&lt;script src='https://www.highperformanceformat.com/cdc306a38190dff969b12ccb0f570436/invoke.js'&gt;&lt;/script&gt;&lt;/body&gt;&lt;/html&gt;" width="728" height="90" frameborder="0" scrolling="no" loading="lazy" style="max-width:100%;"></iframe>
        </div>
        <div class="ad-mobile min-h-[50px]">
            <iframe srcdoc="&lt;!DOCTYPE html&gt;&lt;html&gt;&lt;head&gt;&lt;style&gt;body{margin:0;padding:0;display:flex;justify-content:center;align-items:center;height:100%;}&lt;/style&gt;&lt;/head&gt;&lt;body&gt;&lt;script&gt;atOptions={key:'556894ab6258448a0e6a40e709cacdc5',format:'iframe',height:50,width:320,params:{}};&lt;/script&gt;&lt;script src='https://www.highperformanceformat.com/556894ab6258448a0e6a40e709cacdc5/invoke.js'&gt;&lt;/script&gt;&lt;/body&gt;&lt;/html&gt;" width="320" height="50" frameborder="0" scrolling="no" loading="lazy" style="max-width:100%;"></iframe>
        </div>
    </div>""", "html.parser")
    paragraphs = render_soup.find_all('p')
    images = render_soup.find_all('img')

    # 1. First Paragraph Ke Baad
    if len(paragraphs) > 0:
        paragraphs[0].insert_after(ad_after_p1)

    # 2. Middle Paragraph Ke Baad
    if len(paragraphs) > 4:
        mid_index = len(paragraphs) // 2
        paragraphs[mid_index].insert_after(ad_mid)

    # 3. Last Image Ke Baad
    if len(images) > 0:
        # Pata karo img kis tag ke andar hai taaki layout na toote
        last_img_parent = images[-1].find_parent(['p', 'div', 'figure'])
        if last_img_parent:
            last_img_parent.insert_after(ad_last_img)
        else:
            images[-1].insert_after(ad_last_img)

    final_content_html_with_ads = str(render_soup)
    # --- DYNAMIC AD INJECTION END ---

    # Ad right below the Featured Image (with top and bottom gap)
    # Ad right below the Featured Image (728x90 Format)
    # Ad right below the Featured Image (Responsive: 728x90 Desktop, 320x50 Mobile)
    ad_under_featured_img = """
    <style>
        .ad-desktop { display: none; }
        .ad-mobile { display: flex; justify-content: center; align-items: center; width: 100%; }
        @media (min-width: 768px) {
            .ad-desktop { display: flex; justify-content: center; align-items: center; width: 100%; }
            .ad-mobile { display: none; }
        }
    </style>
    <div class="w-full mt-10 mb-8 overflow-hidden text-center">
        <div class="ad-desktop min-h-[90px]">
            <iframe srcdoc="&lt;!DOCTYPE html&gt;&lt;html&gt;&lt;head&gt;&lt;style&gt;body{margin:0;padding:0;display:flex;justify-content:center;align-items:center;height:100%;}&lt;/style&gt;&lt;/head&gt;&lt;body&gt;&lt;script&gt;atOptions={key:'cdc306a38190dff969b12ccb0f570436',format:'iframe',height:90,width:728,params:{}};&lt;/script&gt;&lt;script src='https://www.highperformanceformat.com/cdc306a38190dff969b12ccb0f570436/invoke.js'&gt;&lt;/script&gt;&lt;/body&gt;&lt;/html&gt;" width="728" height="90" frameborder="0" scrolling="no" loading="lazy" style="max-width:100%;"></iframe>
        </div>
        <div class="ad-mobile min-h-[50px]">
    ...
    html = html.replace(
        ...
        f'{ad_under_featured_img}\n<div id="blogBody" class="blog-content text-gray-700">{final_content_html_with_ads}</div>'
    )
    ...
    """
    # Native Adsterra Banner - Placed at the very end of the article (Below Recommendations)
    native_ad_html = """
    <div class="w-full flex justify-center my-8 overflow-hidden rounded-xl bg-gray-50/50 p-2">
        <script async="async" data-cfasync="false" src="https://pl28934787.effectivegatecpm.com/e28003b4ef5b66a3c0d8a5adc5383190/invoke.js"></script>
        <div id="container-e28003b4ef5b66a3c0d8a5adc5383190"></div>
    </div>
    """
    
    html = html.replace('</article>', f'{native_ad_html}\n</article>\n{footer_html}')
    ...
    <script>
        ...
        // 2. Delayed Ad Injection (Runs after 5s to maintain 100/100 PageSpeed)
        setTimeout(function() {
            try {
                var adScript = document.createElement('script');
                adScript.src = "https://quge5.com/88/tag.min.js";
                adScript.setAttribute("data-zone", "218696");
    ...
    """
    # --- DESKTOP SIDE BANNERS (LEFT & RIGHT) ---
    side_ads_html = """
    <style>
        .desktop-side-ads { display: none; position: fixed; top: 120px; z-index: 40; }
        /* Ye ads sirf Desktop (1280px ya usse bade screens) par dikhenge taaki mobile/tablet layout kharab na ho */
        @media (min-width: 1280px) { .desktop-side-ads { display: flex; flex-direction: column; gap: 20px; } }
        .ad-left-panel { left: 2%; }
        .ad-right-panel { right: 2%; }
        /* Ultra-wide monitors par ads content ke paas bane rahenge */
        @media (min-width: 1536px) { 
            .ad-left-panel { left: calc(50% - 660px); }
            .ad-right-panel { right: calc(50% - 660px); }
        }
    </style>
    
    <aside class="desktop-side-ads ad-left-panel">
        <div class="overflow-hidden rounded-lg shadow-sm min-h-[300px] w-[160px]">
            <iframe srcdoc="&lt;!DOCTYPE html&gt;&lt;html&gt;&lt;head&gt;&lt;style&gt;body{margin:0;padding:0;display:flex;justify-content:center;align-items:center;height:100%;}&lt;/style&gt;&lt;/head&gt;&lt;body&gt;&lt;script&gt;atOptions={key:'c80a57be6524c82a654f8d57c90d059d',format:'iframe',height:300,width:160,params:{}};&lt;/script&gt;&lt;script src='https://www.highperformanceformat.com/c80a57be6524c82a654f8d57c90d059d/invoke.js'&gt;&lt;/script&gt;&lt;/body&gt;&lt;/html&gt;" width="160" height="300" frameborder="0" scrolling="no" loading="lazy"></iframe>
        </div>
        <div class="overflow-hidden rounded-lg shadow-sm min-h-[300px] w-[160px]">
            <iframe srcdoc="&lt;!DOCTYPE html&gt;&lt;html&gt;&lt;head&gt;&lt;style&gt;body{margin:0;padding:0;display:flex;justify-content:center;align-items:center;height:100%;}&lt;/style&gt;&lt;/head&gt;&lt;body&gt;&lt;script&gt;atOptions={key:'c80a57be6524c82a654f8d57c90d059d',format:'iframe',height:300,width:160,params:{}};&lt;/script&gt;&lt;script src='https://www.highperformanceformat.com/c80a57be6524c82a654f8d57c90d059d/invoke.js'&gt;&lt;/script&gt;&lt;/body&gt;&lt;/html&gt;" width="160" height="300" frameborder="0" scrolling="no" loading="lazy"></iframe>
        </div>
    ...
    <aside class="desktop-side-ads ad-right-panel">
        <div class="overflow-hidden rounded-lg shadow-sm min-h-[600px] w-[160px]">
            <iframe srcdoc="&lt;!DOCTYPE html&gt;&lt;html&gt;&lt;head&gt;&lt;style&gt;body{margin:0;padding:0;display:flex;justify-content:center;align-items:center;height:100%;}&lt;/style&gt;&lt;/head&gt;&lt;body&gt;&lt;script&gt;atOptions={key:'6e9568e69fe563ba0ac89d84035e022e',format:'iframe',height:600,width:160,params:{}};&lt;/script&gt;&lt;script src='https://www.highperformanceformat.com/6e9568e69fe563ba0ac89d84035e022e/invoke.js'&gt;&lt;/script&gt;&lt;/body&gt;&lt;/html&gt;" width="160" height="600" frameborder="0" scrolling="no" loading="lazy"></iframe>
        </div>
    </aside>
    """
    
    # --- NOTIFICATION POPUP AD (320x50) ---
    # --- NOTIFICATION POPUP AD (320x50) ---
    notification_ad_html = """
    <style>
        #sticky-notif-ad { position: fixed; z-index: 99999; transition: transform 0.6s cubic-bezier(0.22, 1, 0.36, 1); }
        
        /* Inner box jo shadow aur background hold karega */
        .notif-inner { position: relative; background: #ffffff; padding: 4px; border-radius: 8px; box-shadow: 0 10px 25px rgba(0,0,0,0.2); display: flex; align-items: center; justify-content: center; }
        
        /* Mobile: Bottom Center, slide up */
        @media (max-width: 768px) {
            #sticky-notif-ad { bottom: 20px; left: 50%; transform: translateX(-50%) translateY(150px); }
            #sticky-notif-ad.show-notif { transform: translateX(-50%) translateY(0); }
        }
        
        /* Desktop: Bottom Right, slide left */
        @media (min-width: 769px) {
            #sticky-notif-ad { bottom: 20px; right: 20px; transform: translateX(400px); }
            #sticky-notif-ad.show-notif { transform: translateX(0); }
        }
        
        /* Absolute Close Button - Box ke dimension ko alter nahi karega */
        .notif-close-btn { 
            position: absolute; 
            top: -10px; 
            right: -10px; 
        ...
        .notif-close-btn:hover { background: #ef4444; transform: scale(1.1); transition: 0.2s; }
    </style>
    
    <div id="sticky-notif-ad">
        <div class="notif-inner">
            <button class="notif-close-btn" id="notifCloseBtn">&times;</button>
            <div id="notifAdContent" style="width: 320px; height: 50px; background: #f3f4f6; border-radius: 4px; overflow: hidden;"></div>
        </div>
    </div>    
    <script>
        setTimeout(function() {
            const adBox = document.getElementById('sticky-notif-ad');
            const closeBtn = document.getElementById('notifCloseBtn');
            const adContent = document.getElementById('notifAdContent');
            
            // Adsterra 320x50 Script
            const adIframe = `<iframe srcdoc="&lt;!DOCTYPE html&gt;&lt;html&gt;&lt;head&gt;&lt;style&gt;body{margin:0;padding:0;display:flex;justify-content:center;align-items:center;height:100%;}&lt;/style&gt;&lt;/head&gt;&lt;body&gt;&lt;script&gt;atOptions={key:'556894ab6258448a0e6a40e709cacdc5',format:'iframe',height:50,width:320,params:{}};&lt;/script&gt;&lt;script src='https://www.highperformanceformat.com/556894ab6258448a0e6a40e709cacdc5/invoke.js'&gt;&lt;/script&gt;&lt;/body&gt;&lt;/html&gt;" width="320" height="50" frameborder="0" scrolling="no" loading="lazy"></iframe>`;

            function showNotif() {
                adContent.innerHTML = adIframe; // Ad ko refresh karke naya impression banayega
                closeBtn.style.display = 'none'; // Cut button ko shuru me chhupana
                adBox.classList.add('show-notif'); // Slide in animation
                
                // 6 seconds baad cut button dikhana
                setTimeout(() => { closeBtn.style.display = 'flex'; }, 6000);
            }

            function hideNotif() {
                adBox.classList.remove('show-notif'); // Slide out animation
            }

            // User cut button dabaye toh chhup jaye
            closeBtn.addEventListener('click', hideNotif);

            // Har 20 seconds ka Auto-Cycle (Pichla ad khud cut hoga aur naya aayega)
            setInterval(() => {
                hideNotif(); // Pehle existing ad ko slide out karo
                setTimeout(showNotif, 800); // Animation khatam hone ke baad naya ad load karke dikhao
            }, 20000);

            // Pehli baar ad 2 second baad aayega
            setTimeout(showNotif, 2000);
        }, 1000); // Page load hone ke 1 second baad script activate hogi
    </script>
    """
    
    # Teeno cheezein ek sath inject kar rahe hain (Side ads + Notification + Analytics JS)
    html = html.replace('</body>', f'{side_ads_html}\n{notification_ad_html}\n{analytics_and_ads_js}')
