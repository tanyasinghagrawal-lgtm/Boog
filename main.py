...
async def get_query_embedding(query_text: str) -> Optional[List[float]]:
    """User ki search query ko vector format me convert karne ke liye."""
    url_template = "https://generativelanguage.googleapis.com/v1beta/models/gemini-embedding-001:embedContent?key={api_key}"
    payload = {
...
# --- NEW: Smart Chunking & AI Content Generators ---
def get_smart_chunks(text: str, max_words: int = 1000) -> List[str]:
    """Sentence boundaries (Full stop '.') ke hisaab se text ko todta hai."""
    import re
    ...
    for sentence in sentences:
        ...
        if current_word_count + words_count > max_words and current_chunk:
            chunks.append(" ".join(current_chunk))
            current_chunk = [sentence]
            current_word_count = words_count
        else:
...
async def background_process_new_article_ai(wp_slug: str, html_content: str, title: str):
    """Background me naye article ke vectors aur AI Summary/Facts generate karke DB me save karega."""
    logger.info(f"⏳ Starting background AI processing (Vectors & Content) for: {wp_slug}")
    
    soup = BeautifulSoup(html_content, 'html.parser')
    clean_text = soup.get_text(separator=" ", strip=True)
    
    if len(clean_text) < 50:
        logger.warning(f"⚠️ Article too short for AI processing: {wp_slug}")
        return
        
    try:
        async with db_pool.acquire() as conn:
            # 1. Generate Vectors (Smart Chunking)
            vec_exists = await conn.fetchval("SELECT 1 FROM article_vectors WHERE wp_slug = $1 LIMIT 1", wp_slug)
            if not vec_exists:
                chunks = get_smart_chunks(clean_text, max_words=1000)
                for i, chunk_text in enumerate(chunks):
                    # Hum get_query_embedding ka use kar rahe hain jo pichle step me banaya tha
                    vector = await get_query_embedding(chunk_text) 
                    if vector:
                        await conn.execute(
                            """INSERT INTO article_vectors (wp_slug, chunk_index, chunk_text, embedding) 
                               VALUES ($1, $2, $3, $4) ON CONFLICT DO NOTHING""",
                            wp_slug, i, chunk_text, str(vector)
                        )
                        await asyncio.sleep(1) # Prevent API rate limits
                        
            # 2. Generate Summary & Facts
            row = await conn.fetchrow("SELECT ai_summary, ai_facts FROM articles WHERE wp_slug = $1", wp_slug)
            if row:
                if not row['ai_summary']:
                    summary = await generate_ai_content(clean_text, title, "summary")
                    if summary:
                        await conn.execute("UPDATE articles SET ai_summary = $1 WHERE wp_slug = $2", summary, wp_slug)
                        if wp_slug in ARTICLE_CACHE: ARTICLE_CACHE[wp_slug]['ai_summary'] = summary
                
                if not row['ai_facts']:
                    facts = await generate_ai_content(clean_text, title, "facts")
                    if facts:
                        await conn.execute("UPDATE articles SET ai_facts = $1 WHERE wp_slug = $2", facts, wp_slug)
                        if wp_slug in ARTICLE_CACHE: ARTICLE_CACHE[wp_slug]['ai_facts'] = facts

        logger.info(f"✅ Background AI processing completed for: {wp_slug}")
    except Exception as e:
        logger.error(f"❌ Failed in background AI processing for {wp_slug}: {e}")
...
async def scrape_and_save_article(wp_slug: str, client: httpx.AsyncClient) -> bool:
    ...
    try:
        ...
        # --- NEW: Trigger Background AI Processing (Vectors, Summary, Facts) ---
        ...
        async with db_pool.acquire() as conn:
            ...
            WITH RankedChunks AS (
                SELECT
                    ...
                    ROW_NUMBER() OVER(PARTITION BY a.wp_slug ORDER BY v.embedding <=> $1::vector ASC) as rn
                FROM article_vectors v
                JOIN articles a ON v.wp_slug = a.wp_slug
            )
            SELECT wp_slug, title, seo_description, featured_hash, distance
            FROM RankedChunks
            WHERE rn = 1
